defmodule LazyDoc.TaskTest do
  ...
  test "extract the names of a module" do
    ...
    names_with_clauses_joined = functions |> TaskLazyDoc.join_code_from_clauses()
  ...
  test "docs per module" do
    docs_per_module = TaskLazyDoc.docs_per_module([LazyDoc])
    ...
    docs_per_module = TaskLazyDoc.docs_per_module([LazyDoc.ExampleModule])
  ...
  test "filter undocumented docs" do
    ...
    name_module =
      TaskLazyDoc.extract_names(ast)
      |> Enum.map(fn {:module, module_name, module_ast, functions} ->
        {module_name |> Module.concat(), module_ast,
         TaskLazyDoc.join_code_from_clauses(functions)}
      end)
    ...
    docs_per_module =
      TaskLazyDoc.docs_per_module([LazyDoc.ExampleModule])
      |> Enum.map(fn {module, {module_doc, func_docs}} ->
        {module, {module_doc, TaskLazyDoc.group_docs_different_arities(func_docs)}}
      end)
      |> Enum.at(0)

    {_module, _module_ast, functions_to_document} =
      TaskLazyDoc.filter_undocumented_functions(name_module, docs_per_module)
